...
//Static variables:
static int planeP = 165, planeLeft=0, planeRight=0;
static int bulletsX = planeP,bulletsY = 0;
static int gameSpeed=0, play=0, oldGame=0;
static int point=0, life=3, earthLevel=100, nAsteroid=2;
static int presentTime=0, levelTime=0;
static int mainMenuSelection=1, pages=0, backGround = 1, starMove=0;
...
void bulletsDynamic()
{
    ...
    glVertex2d(planeP+0,border+35);
    glVertex2d(planeP+5,border+45);
    glVertex2d(planeP+10,border+35);
    glEnd();
    glBegin(GL_TRIANGLES);
    glColor3ub(255,255,255);
    glVertex2d(planeP+15,border+35);
    glVertex2d(planeP+20,border+45);
    glVertex2d(planeP+25,border+35);
    glEnd();
    glBegin(GL_TRIANGLES);
    glColor3ub(255,255,255);
    glVertex2d(planeP+95,border+35);
    glVertex2d(planeP+100,border+45);
    glVertex2d(planeP+105,border+35);
    glEnd();
    glBegin(GL_TRIANGLES);
    glColor3ub(255,255,255);
    glVertex2d(planeP+110,border+35);
    glVertex2d(planeP+115,border+45);
    glVertex2d(planeP+120,border+35);
    glEnd();
}
...
void planeBody()
{
    ...
    glEnd();
    drawLineCircle(planeP+60,border+40,10,10,255-life*80,life*40,100+life*30);
...
int enemyKilled()
{
    for(int i=0; i<nAsteroid; i++)
    {
        if(((asteroids[i].x<=bulletsX && asteroids[i].x+asteroids[i].sz>=bulletsX)||(asteroids[i].x<=bulletsX+10 && asteroids[i].x+asteroids[i].sz>=bulletsX+10))
                || ((asteroids[i].x<=bulletsX+15 && asteroids[i].x+asteroids[i].sz>=bulletsX+15)||(asteroids[i].x<=bulletsX+25 && asteroids[i].x+asteroids[i].sz>=bulletsX+25))
                || ((asteroids[i].x<=bulletsX+95 && asteroids[i].x+asteroids[i].sz>=bulletsX+95)||(asteroids[i].x<=bulletsX+105 && asteroids[i].x+asteroids[i].sz>=bulletsX+105))
                || ((asteroids[i].x<=bulletsX+110 && asteroids[i].x+asteroids[i].sz>=bulletsX+110)||(asteroids[i].x<=bulletsX+120 && asteroids[i].x+asteroids[i].sz>=bulletsX+120)))
            if(asteroids[i].y<=bulletsY)
            {
                return i;
            }
    }
